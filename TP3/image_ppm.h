...
int pixelNoirVoisinage(OCTET *ImgIn, int x, int y, int nW, int nH) {
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            int new_x = x + dx;
            int new_y = y + dy;
            if (new_x >= 0 && new_x < nW && new_y >= 0 && new_y < nH) {
                if (ImgIn[indicePixel(new_x, new_y, nW)] == 0) {
                    return 1;
                }
            }
        }
...
int pixelBlancVoisinage(OCTET *ImgIn, int x, int y, int nW, int nH) {
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            int new_x = x + dx;
            int new_y = y + dy;
            if (new_x >= 0 && new_x < nW && new_y >= 0 && new_y < nH) {
                if (ImgIn[indicePixel(new_x, new_y, nW)] == 255) {
                    return 1;
                }
            }
        }
